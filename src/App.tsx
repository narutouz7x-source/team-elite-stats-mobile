...
export function App() {
  ...
  return <div className="app-shell">
    ...
    <main>
      ...
      {tab === 'matches' && <><PageHeading title="Match History" subtitle="Every recorded result, synced from the OG ELITE ledger."/><StagePicker stages={stages} selected={selectedStage} onSelect={setSelectedStage}/><MatchList matches={visibleMatches} onView={setSelectedMatch}/></>}
      ...
      {tab === 'crew' && <><section className="mobile-content-hero"><div><span>OG ELITE / COMMUNITY</span><h1>OUR CREW</h1><p>Meet the creators and personalities building the OG ELITE community.</p></div></section><section className="mobile-creator-grid">{creators.length?creators.map(creator=><article className="mobile-creator-card" key={creator.id}><div className="mobile-creator-image">{creator.imageUrl?<img src={mediaUrl(creator.imageUrl)} alt={creator.name} loading="eager" decoding="async" referrerPolicy="no-referrer" onError={e=>{e.currentTarget.style.display='none'}}/>:<div>{creator.name.slice(0,2).toUpperCase()}</div>}{creator.featured&&<small>FEATURED</small>}<div className="mobile-creator-name"><h2>{creator.name}</h2></div>{creator.handle&&<span className="mobile-creator-handle">{creator.handle}</span>}</div><div className="mobile-creator-body"><p>{creator.bio||'OG ELITE community creator.'}</p><div className="mobile-creator-links">{creator.youtube&&<a href={creator.youtube} target="_blank" rel="noreferrer" aria-label={`Open ${creator.name}'s YouTube`}><span className="creator-platform-logo creator-platform-youtube">▶</span><span>{creator.handle||'YOUTUBE'}</span></a>}{creator.instagram&&<a href={creator.instagram} target="_blank" rel="noreferrer" aria-label={`Open ${creator.name}'s Instagram`}><span className="creator-platform-logo creator-platform-instagram">◎</span><span>{creator.handle||'INSTAGRAM'}</span></a>}{creator.discord&&<a href={creator.discord} target="_blank" rel="noreferrer" aria-label={`Open ${creator.name}'s Discord`}><span className="creator-platform-logo creator-platform-discord">◉</span><span>{creator.handle||'DISCORD'}</span></a>}</div></div></article>):<div className="mobile-empty-feature"><b>CREATORS WILL APPEAR HERE</b></div>}</section></>}
